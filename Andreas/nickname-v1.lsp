...
;"kill" - kills the contract 
{
	...
	[0x0] "reg"
	[0x20] "nick"
	(call 0 @@0x10 0 0x0 0x40 0x0 0x20) ;Register with DOUG
...
}
{
	...
	(call (- (GAS) 100) @@0x10 0 0x40 0x40 0x80 0x20)
	...
	(when (AND (= @0x0 "reg") (> @0x20 0x20)) ;Command "reg" and make sure it won't overwrite important data 
		{
			...
			(call (- (GAS) 100) @0x80 0 0x40 0x40 0x40 0x20)
			...
			(when (= @0x40 0) 
				{
					[0x40] "regmem"
					[0x60] (CALLER)
					(call 0 @0x80 0 0x40 0x40 0x0 0x20)
				}
			)
			...
			(when @@(caller)
				{
                    ...
                    [0x40] "delmem"
					[0x60] (CALLER)
					(call (- (GAS) 100) @0x80 0 0x40 0x40 0x0 0x20)
	...
	(when (AND (= @0x0 "dereg") (> @0x20 0x20) ) ; When de-regging by name.
		{
            ...
            [0x40] "check"
			[0x60] @@ @0x20
			(call 0 @0x80 0 0x40 0x40 0x100 0x20)
