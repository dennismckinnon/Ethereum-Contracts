			...
			(while (> @0x0 0) ;Loop until head is reached or the next pointer = 0
				{
					...
					[0x20]"check"
					(call (- (GAS) 100) @0x0 0 0x20 0x20 0x40 0x20)
					...
					(when (= @0x40 2)
						{
							...
							(when (= @0x40 "doug")
								{
									(call (- (GAS) 100) @@0x10 0 0 0 0 0) ;clear the name registration
									[0x1A0] "claim"
									(call (- (GAS) 100) @@"doug" 0 0x1A0 0x20 0 0) ;Call new doug to tell him to claim name (It doesn't matter if new doug responds to this)
								}
							)
	...
	(when (= @0x20 "req") ;Request made
		{
			...
			(if (= @@"doug" (ADDRESS))
				...
				{	;If this doug is not the head of the doug chain pass the request forward to doug
					(call (- (GAS) 100) @@"doug" 0 0x20 0x40 0x60 0x40) ;Get the answer from Doug
					...
					(if (= @@"pollcodes" 0)
						{
							...
							[0x20]"inlist"
							(call (- (GAS) 100) (ADDRESS) 0 0x20 0x40 0x0 0x20)
						...
						}
						{
							...
							[0x20]"inlist"
							(call (- (GAS) 100) @@"doug" 0 0x20 0x40 0 0x20)
