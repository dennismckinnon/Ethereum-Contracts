...

{
	...
	[[0x10]] 0x0 						;Hardcode in a first DOUG
	[[0x11]] 32						 	;Number of segments per item
	[[0x12]] 0x20 						;infohash list pointer
	[[0x13]] (* (EXP 0x100 21) @@0x11) 	;Where to start the inforhash look up. 20 bytes hence 21
	[[0x14]] 0 							;count number of entries
	...
	(when (= @0x20 "moddbe")
		{
			...
			[0xA0] (+ @0x20 @@0x13) ;Offset position for name look up
			...
			(if (= @@ @0xA0 0)
				{
					...
					[[0x12]](+ @@0x12 @@0x11) ;increment pointer
					[[0x14]](+ @@0x14 1) ;increment counter
	...
	(when (= @0x20 "deldbe")
		{
			...
			[0x80] (- @@0x12 1) ;Locator for item to move over

			[[@0xA0]] 0; Clear out

			[0xA0] (+ @@ @0x80 @@0x13) ;This is the address the infohash pointer
			(unless (= @0x60 @0x80)
				[[@0xA0]] @0x60 ;Copy the new locator for this item
			)
			
			(for [0x100]0 (< @0x100 @@0x11) [0x100](+ @0x100 1)
				{
			...
			[[0x11]] @0x80 ;Change the data pointer
			[[0x14]](- @@0x14 1);Decrement counter
		}
	)
}